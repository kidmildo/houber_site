...
(현재 항목: HOME · 회사소개 · 사업목표 · 사업내용 · 소식 · 문의하기)
...
인스타그램·블로그·유튜브 주소는 **한 페이지 안에서 세 곳**에 있습니다.
셋 다 같은 값을 유지해야 합니다.

1. `#news` 섹션의 채널 카드 3장 (`channel-card`)
2. `#contact` 섹션 아래의 아이콘 줄 (`sns-row`)
3. (푸터에는 SNS가 없습니다 — 관계사 목록만 있습니다)

현재 주소:

| 채널 | 주소 |
| --- | --- |
| 인스타그램 | `https://www.instagram.com/osaka_houber/` |
| 네이버 블로그 | `https://blog.naver.com/houber` |
| 유튜브 | `https://www.youtube.com/@Houber_Official` |

한국어·일본어 두 파일 모두 고쳐야 합니다.

> **최신 글·영상을 카드로 보여주려면**
> 지금은 채널 안내만 있습니다. 유튜브 영상 주소와 블로그 글 주소를 정해서
> 넣으면 카드가 늘어납니다. 단 **블로그 글은 자동으로 따라오지 않습니다** —
> 네이버가 브라우저의 직접 읽기를 막아두기 때문에, 글 제목과 주소를
> HTML에 직접 적어두고 새 글이 나오면 교체하는 방식이 됩니다.

### 3-8. 관계사(GROUP) 링크

HOUBER 의 두 서비스 사이트로 가는 링크입니다. **12장 전부의 푸터**와,
HOUBER 사업 상세 페이지(=팝업) 안의 `detail-sites` 블록에 있습니다.

| 사이트 | 주소 |
| --- | --- |
| 부동산 매물 | `https://www.houber-home.com` |
| 비즈니스 | `https://houber-business.com/ko/` (한국어) |
| 비즈니스 | `https://houber-business.com/` (일본어 — **주소 확인 필요**) |

> ⚠️ 일본어 페이지에서 쓰는 비즈니스 사이트 주소는 **아직 확인되지 않았습니다.**
> 일본어 전용 주소가 따로 있으면 `ja/index.html` 과
> `ja/business/houber.html` 두 곳을 고쳐주세요.
...
- [x] ~~**2. 행정서사 등록번호 · 회원번호 입력**~~ ✅ 완료
      `行政書士登録番号 第24261578号`
      `大阪行政書士会 会員番号008980`
      → 12장 전부의 푸터 `footer__license` 와
...
**12장 전부의 푸터에 같은 값**이 들어가 있습니다. 하나만 고치면 값이 어긋나므로
바꿀 때는 반드시 전부 함께 갱신하세요.
...
```bash
grep -rl "第24261578号" . --include="*.html" \
  | xargs sed -i '' 's/第24261578号/새번호/g'
```
